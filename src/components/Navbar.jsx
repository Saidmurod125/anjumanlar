...
function Navbar() {
  ...
  return (
    ...
    <>
      <nav className="fixed top-0 left-0 right-0  p-4 mx-auto bg-white dark:text-white md:px-14 max-w-screen-2xl dark:bg-gray-900 ">
        <div className="container flex items-center justify-between mx-auto text-lg font-medium ">
          ...
          <div className="items-center hidden space-x-12 md:flex">
            <a
              href="/"
              className="items-center hidden lg:flex hover:text-secoundary"
            >
              <GrLanguage className="mr-2" />
              Language
            </a>
            <button className="px-4 py-2 transition-all duration-300 rounded bg-secoundary hover:bg-indigo-600 hover:text-secoundary">
              Sign up
            </button>
          </div>
      ...
      <div
        className={` space-y-4 px-4 pt-24 text-xl  pb-5  bg-secoundary ${
          isMenuOpen ? " fixed block  top-0  right-0 left-0 " : "hidden"
        } `}
      >
        {navItems.map(({ link, path }) => (
          <li key={link}>
